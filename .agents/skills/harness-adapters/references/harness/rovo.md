...
A positional brief exits before work begins, and `--startup-receipt` requires prompt-free interactive mode, so neither supports this adapter's brief delivery.
`../../../docs/verification/rovo.md` owns the launch evidence.
...
An unresolved readiness or delivery gate returns nonzero and retains the endpoint and ownership records for supervision and ordinary teardown.
Spawn reports `unreadable: rovo startup unconfirmed` unless the backend positively proves departure, which produces a `failed:` report.
...
Readiness can succeed from the welcome banner while the delivery gate remains unconfirmed, as the [Herdr verification](../../../../../docs/verification/rovo.md#backend-liveness) records.
Apply the startup-preservation rule above even when the worker has visibly begun the brief.
