...
`GEMINI_CLI=1` is required when ancestry cannot identify the Node bundle, so `../../../../../bin/fm-harness.sh` checks it BEFORE `CLAUDECODE`.
...
Ancestry support depends on Node's process title.
The `node` and `node-MainThread` titles reach the existing interpreter matcher.
The `MainThread` title measured on Node v24.20.0 does not, so that process shape requires the marker.
