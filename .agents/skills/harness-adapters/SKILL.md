...
| Composer | Two solid `─` rules around a shell `>` prompt, no vertical borders. Idle is a lone `>`; pending is `> hello`. `cursor_y` sits on the `>` row. The shared classifier treats that separated-plus-`>` shape as a proven container; a bare `>` without the rules stays a dead shell. `>` replaces only pi's identity requirement: the region must still pass the same `FM_COMPOSER_PI_MAX_LINES` bound, and `$`, `%`, and `#` are NOT verified in this shape. |
