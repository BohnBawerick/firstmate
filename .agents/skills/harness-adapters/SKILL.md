...
| agy | `--model <model>` | none | Verified 2026-08-14 on agy 1.1.12. Firstmate always pins `--model gemini-3.1-pro-high` and never emits `--effort`: the model id already encodes effort, and `--model gemini-3.1-pro-high --effort low` made 1.1.12 fall back to Gemini 3.6 Flash High. Requested effort is recorded in task metadata only. |
...
| Trust dialog | `Do you trust the contents of this project?` with `Yes, I trust this folder` preselected, accepted by Enter. `--dangerously-skip-permissions` does NOT suppress it. The decision is persisted as the exact worktree path in `~/.gemini/antigravity-cli/settings.json` `trustedWorkspaces`, so every fresh treehouse path shows it again. `fm-spawn` sends Enter only when that dialog is visible. |
...
| Composer | Two solid `─` rules around a shell `>` prompt, no vertical borders. Idle is a lone `>`; pending is `> hello`. `cursor_y` sits on the `>` row. The shared classifier treats that separated-plus-`>` shape as a proven container; a bare `>` without the rules stays a dead shell. |
