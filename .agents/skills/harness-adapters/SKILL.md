...
| codex | `--model <model>` | `-c 'model_reasoning_effort="<low\|medium\|high\|xhigh\|max>"'` | Verified 2026-08-29 on codex-cli 0.150.1. The installed binary schema contains `model_reasoning_effort`, the active config uses it, and an invalid-effort 400 enumerates none/minimal/low/medium/high/xhigh/max. `max` is emitted. |
