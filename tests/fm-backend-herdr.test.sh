...
test_stale_registration_over_a_shell_only_pane_is_agent_free() {
  ...
  sleep_bin=$(idle_shell_fixture_binary) || fail "sleep not found"
...
test_registered_agent_with_an_empty_foreground_remains_unreadable() {
  local sleep_bin shell_pid out
  sleep_bin=$(idle_shell_fixture_binary) || fail "sleep not found"
  # A real root without foreground evidence still cannot prove exit.
  "$sleep_bin" 300 &
  shell_pid=$!
  ...
  kill "$shell_pid" 2>/dev/null || true
  [ "$out" = "unknown unreadable refused" ] \
    || fail "an empty foreground list must remain unreadable, got '$out'"
  pass "herdr stale registration: an empty foreground list remains unreadable even with a real root"
}
...
case "$*" in
  ...
  "-p 200 -o args=")
    [ "${FM_FAKE_ARGS_UNREADABLE:-0}" != 1 ] || exit 1
    printf 'treehouse\n' ;;
  "-p 300 -o args=") printf 'bash\n' ;;
  *) exit 1 ;;
esac
SH
  chmod +x "$dir/ps"
...
test_agent_state_reports_unreadable_descendant_arguments() {
  local harness dir log resp fb out
  for harness in pi claude; do
    dir="$TMP_ROOT/args-unreadable-$harness"; mkdir -p "$dir/responses"
    log="$dir/log"; resp="$dir/responses"; : > "$log"
    printf '%s\n' '{"result":{"pane":{"pane_id":"w1:p2"}}}' > "$resp/1.out"
    ...
    fb=$(make_herdr_fakebin "$dir")
    out=$(PATH="$fb:$PATH" FM_HERDR_LOG="$log" FM_HERDR_RESPONSES="$resp" \
      FM_FAKE_ARGS_UNREADABLE=1 FM_HERDR_PS_BIN="$dir/ps" \
      bash -c '. "$0/bin/fm-backend.sh"; fm_backend_agent_state herdr fmtest:w1:p2' "$ROOT")
    [ "$out" = unreadable ] \
      || fail "unreadable descendant arguments must not prove $harness exited, got '$out'"
  done
  pass "fm_backend_herdr_agent_state: missing descendant arguments cannot prove exit"
}

test_agent_state_reports_unreadable_pi_process_state() {
  local dir log resp fb out
  ...
  out=$(PATH="$fb:$PATH" FM_HERDR_LOG="$log" FM_HERDR_RESPONSES="$resp" \
    ...
    FM_BACKEND_HERDR_DEPARTED_PI_POLLS=1 \
    bash -c '. "$0/bin/fm-backend.sh"; fm_backend_agent_state herdr fmtest:w1:p2' "$ROOT")
  [ "$out" = unreadable ] || fail "an unreadable Pi process state must report unreadable, got '$out'"
  pass "fm_backend_herdr_agent_state: an unreadable Pi state reports unreadable"
}

test_agent_state_reports_unreadable_without_complete_shell_lineage() {
  ...
  for lineage in missing-root missing-intermediary unrelated-foreground \
    ...
    out=$(PATH="$fb:$PATH" FM_HERDR_LOG="$log" FM_HERDR_RESPONSES="$resp" \
      ...
      bash -c '. "$0/bin/fm-backend.sh"; fm_backend_agent_state herdr fmtest:w1:p2' "$ROOT")
    [ "$out" = unreadable ] \
      || fail "an unproved $lineage pane-shell lineage must report unreadable, got '$out'"
  done
  pass "fm_backend_herdr_agent_state: incomplete or contradictory shell lineage reports unreadable"
}

test_agent_state_reports_unreadable_pi_process_proof() {
  local dir log resp fb out
  ...
  out=$(PATH="$fb:$PATH" FM_HERDR_LOG="$log" FM_HERDR_RESPONSES="$resp" \
    ...
    bash -c '. "$0/bin/fm-backend.sh"; fm_backend_agent_state herdr fmtest:w1:p2' "$ROOT")
  [ "$out" = unreadable ] || fail "an unreadable Pi process proof must report unreadable, got '$out'"
  pass "fm_backend_herdr_agent_state: an unreadable process proof reports unreadable"
}
