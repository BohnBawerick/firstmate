...
EOF
  ...
  [ ! -s "$case_dir/nm-abort.log" ] || fail "coarse ledger rows authorized an unverified run abort"
  pass "a parked unfetched run without explicit ownership proof is left untouched"
}
...
test_parked_unfetched_run_requires_explicit_ownership() {
  ...
  for proof in custody submitted mismatched-submitted; do
    ...
    write_meta "$case_dir" no-mistakes ship
    land_shippable_commit "$case_dir"
    anchor_head=$(git -C "$case_dir/wt" rev-parse HEAD)
    advanced_short=$(make_unfetched_pipeline_heads "$case_dir")
    assert_head_absent_from_worktree "$case_dir/wt" "$advanced_short" "explicit-$proof"
    status_payload=$(parked_axi_status_toon fm/task-x1 "$advanced_short")
    sync_payload=
    case "$proof" in
      custody) status_payload="$status_payload
branch_sync:
...
EOF
  ...
  assert_absent "$case_dir/nm-runs.log" \
    "teardown must not consult coarse ledger rows for unfetched-head ownership"
  assert_absent "$case_dir/nm-abort.log" \
...
EOF
  ...
  assert_absent "$case_dir/nm-runs.log" \
    "teardown must not infer ownership from a neighboring terminal row"
  assert_absent "$case_dir/nm-abort.log" \
