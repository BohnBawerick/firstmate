...
test_herdr_agent_state_preserves_husk_classifier() {
  ...
  for row in 'dead missing' 'no-agent dead' 'stale-agent dead' 'live alive' 'unknown unreadable'; do
    pane_state=${row%% *}
    expected=${row#* }
  ...
  out=$(bash -c '. "$0/bin/backends/herdr.sh"
    fm_backend_herdr_pane_agent_state() { printf unknown; }
    fm_backend_herdr_server_running_state() { printf stopped; }
    fm_backend_herdr_agent_state "sess:p1"' "$ROOT")
  [ "$out" = missing ] || fail "a confirmed stopped Herdr server should map to missing, got '$out'"
