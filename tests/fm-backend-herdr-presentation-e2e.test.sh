...
finish_concurrent_recovery() {  # <id> <home> <status> <stdout> <stderr>
  ...
  spawn_task "$id" "$home" "$RECOVERY_PROJECT_DIR" > "$out" 2> "$err" \
    || fail "recovery $id retry failed after the concurrent attempts completed: $(cat "$err")"
  pass "real Herdr lab: recovery $id succeeds after the contended session lock is released"
}
...
# Two homes recovering concurrently each replace only their own exact husk.
# The session lock wait is bounded, so retry a refused recovery only after
# both initial attempts have exited and released their locks.
...
finish_concurrent_recovery "$PRIMARY_WAVE_ID" "$HOME_DIR" "$PRIMARY_WAVE_STATUS" "$TMP_ROOT/primary-wave-resume.out" "$TMP_ROOT/primary-wave-resume.err"
finish_concurrent_recovery "$BRAVO_WAVE_ID" "$SECOND_HOME_B" "$BRAVO_WAVE_STATUS" "$TMP_ROOT/bravo-wave-resume.out" "$TMP_ROOT/bravo-wave-resume.err"
