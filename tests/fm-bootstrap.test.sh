...
test_session_provider_backends_do_not_require_tmux() {
  local backend cli case_dir fakebin out base_without_sessions
  base_without_sessions=$(fm_test_base_path_sans "$BASE_PATH" herdr zellij cmux tmux)
  ...
  while IFS='^' read -r backend cli; do
    ...
    out=$(PATH="$fakebin:$base_without_sessions" FM_HOME="$case_dir/home" FM_ROOT_OVERRIDE="$case_dir/home" \
      FM_FAKE_TREEHOUSE_LEASE_HELP=1 "$ROOT/bin/fm-bootstrap.sh")
...
test_session_provider_backends_gate_own_cli_not_tmux() {
  local backend cli case_dir fakebin out missing base_without_sessions
  base_without_sessions=$(fm_test_base_path_sans "$BASE_PATH" herdr zellij cmux tmux)
...
agy suffixed model with matching effort is accepted^{"rules":[{"when":"agy work","use":{"harness":"agy","model":"gemini-3.1-pro-high","effort":"high"}}]}^empty^
