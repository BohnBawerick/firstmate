...
test_no_mistakes_dod_wording() {
  ...
  assert_grep "include this brief's \`## Captain's intent\` and \`## Firstmate spec\`" "$brief" \
    "no-mistakes DOD must preserve the complete accepted specification"
  assert_grep "with their provenance labels" "$brief" \
    "no-mistakes DOD must distinguish requirement provenance"
  assert_grep "plus every later accepted requirement" "$brief" \
    "no-mistakes DOD must preserve accepted steers"
  assert_grep "unaccepted worker decisions and tradeoffs" "$brief" \
    "no-mistakes DOD must exclude unaccepted worker choices"
...
# The worker starts its own no-mistakes run the moment implementation is
# committed. The generated brief is the worker-facing contract; this test
# pins both directions of that change through the public scaffold, not by
# reading fm-brief.sh source.
...
test_status_protocol_shows_documented_decision_key_placement() {
  ...
  for kind in ship scout dreamer secondmate; do
    ...
    brief="$home/data/$id/brief.md"
    # shellcheck disable=SC2016 # Literal backticks and braces must remain unexpanded.
    assert_grep '`needs-decision [key=<slug>] [at=<epoch>]: {summary of options}`' "$brief" \
      "$kind brief did not show the documented before-colon key on needs-decision"
    # shellcheck disable=SC2016 # Literal backticks and braces must remain unexpanded.
    assert_grep '`resolved [key=<slug>] [at=<epoch>]: {how it cleared}`' "$brief" \
