...
record_daemon_lock_without_startup_stamp() {  # <dir> <pid> [identity]
...
test_hook_away_daemon_allows_between_watcher_cycles() {
  ...
  sleep 60 &
  pid=$!
  record_daemon_lock_without_startup_stamp "$dir" "$pid" || {
    kill "$pid" 2>/dev/null || true
    wait "$pid" 2>/dev/null || true
...
test_hook_away_mode_blocks_on_dead_daemon() {
  ...
  dead=$(nonexistent_pid)
  record_daemon_lock_without_startup_stamp "$dir" "$dead" "dead daemon identity"
  out=$(run_hook "$dir" false); status=$?
...
test_hook_away_mode_blocks_on_pid_reused_daemon() {
  ...
  record_daemon_lock_without_startup_stamp "$dir" "$pid" "some other process identity"
  out=$(run_hook "$dir" false); status=$?
  kill "$pid" 2>/dev/null || true
  wait "$pid" 2>/dev/null || true
...
# accepts an explicit FM_AWAY_TICK_GRACE for that cadence, but still blocks a
# dead daemon or a tick older than the configured bound.

test_hook_away_daemon_allows_beacon_within_configured_tick_grace() {
  ...
  # 400s exceeds the default tick budget but remains within an explicitly
  # configured 660s budget for a daemon running a longer polling cycle.
  beat=$(( $(date +%s) - 400 ))
  fm_touch_epoch "$beat" "$dir/state/.last-watcher-beat"
  out=$(FM_GUARD_GRACE='' FM_AWAY_TICK_GRACE=660 FM_POLL=600 run_hook "$dir" false); status=$?
  kill "$pid" 2>/dev/null || true
  wait "$pid" 2>/dev/null || true
  expect_code 0 "$status" "a live daemon with a beacon within the configured daemon tick grace must not block"
  [ -z "$out" ] || fail "away-mode daemon within configured daemon tick grace still produced a block banner: $out"
  pass "fm-turnend-guard: away-mode beacon freshness uses the configured daemon tick grace, not the default tick budget"
}

test_hook_away_daemon_blocks_dead_daemon_despite_configured_tick_grace() {
  local dir dead out status
  dir=$(make_away_home_between_cycles "$TMP_ROOT/hook-afk-poll-grace-dead-daemon")
  dead=$(nonexistent_pid)
  record_daemon_lock_without_startup_stamp "$dir" "$dead" "dead daemon identity"
  out=$(FM_GUARD_GRACE='' FM_AWAY_TICK_GRACE=660 FM_POLL=600 run_hook "$dir" false); status=$?
  expect_code 2 "$status" "a wider configured daemon tick grace must not paper over a dead daemon"
  assert_contains "$out" "$AWAY_REQUIRED_REASON" "away-mode block must point at the daemon, not normal supervision"
  pass "fm-turnend-guard: a dead away-mode daemon still blocks under the configured daemon tick grace"
}

test_hook_away_daemon_blocks_beacon_older_than_configured_tick_grace() {
  local dir pid out status beat
  ...
  # 700s exceeds the configured daemon tick grace of 660s, so a
  ...
  kill "$pid" 2>/dev/null || true
  wait "$pid" 2>/dev/null || true
  expect_code 2 "$status" "a beacon older than the configured daemon tick grace must still block"
  assert_contains "$out" "$AWAY_REQUIRED_REASON" "away-mode block must point at the daemon, not normal supervision"
  pass "fm-turnend-guard: the configured daemon tick grace is bounded, not unlimited"
}
...
test_hook_no_afk_ignores_poll_derived_grace() {
  ...
  # 400s would be within the wider grace a long-poll auto-arm would
