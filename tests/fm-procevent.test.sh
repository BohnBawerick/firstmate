...
(
  export FM_PROCEVENT_OWNER_LEASE_SECONDS="$PROOF_LEASE_SECONDS"
  export FM_PROCEVENT_OWNER_CHECK_SECONDS="$PROOF_CHECK_SECONDS"
  export FM_HOME="$HORPHAN"
  exec "$ROOT/bin/fm-procevent.sh" reconcile
) >/dev/null &
ORPHAN_RECONCILER=$!
wait "$ORPHAN_RECONCILER" || fail "the orphan fixture reconciler failed"
...
# The recorded reconciler has exited and the listener has another parent.
# Linux can adopt it into a subreaper such as systemd rather than PID 1.
orphan_ppid=$(ps -o ppid= -p "$ORPHAN_PID" 2>/dev/null | tr -d '[:space:]')
case "$orphan_ppid" in
  ''|*[!0-9]*) fail "the listener parent could not be read" ;;
esac
[ "$orphan_ppid" -ge 1 ] && [ "$orphan_ppid" != "$ORPHAN_RECONCILER" ] \
  || fail "the listener was not reparented away from reconciler $ORPHAN_RECONCILER (ppid $orphan_ppid)"
