...
test_merged_observation_reaches_existing_owners() {
  local home out
  home=$(new_home merged-interrupted-owners)
  forge_home "$home"
  wrap_forge "$home"
  record "$home" duplicate 8 open conflicting
  mutate_record "$home" delivery '.records[0].observation.state = "merged" | .records[0].notified = ["owner-a"]'
  mutate_record "$home" duplicate '.records[0].seen = ["owner-b"] | .records[0].notified = ["owner-b"]'
  printf 'down\n' > "$home/forge/fault"
  with_home "$home" "$ROOT/bin/fm-contributions.sh" poll >/dev/null || fail 'merged recovery poll failed'
  jq -e '.records[0] | .observation.state == "merged" and .error == null
    and .seen == ["owner-b"] and .notified == ["owner-b"]' "$home/data/duplicate/contributions.json" >/dev/null \
    || fail 'existing owner did not settle with its own acknowledgment state'
  out=$(bearings "$home")
  printf '%s' "$out" | jq -e '.contributions.counts.fleet == 0 and .contributions.counts.nobody == 1' >/dev/null \
    || fail "the shared merged contribution remained actionable: $out"
  [ ! -s "$home/forge/calls" ] || fail 'merged recovery re-read the forge'
  pass 'merged observations recover across existing owners without resetting acknowledgments'
}

test_merged_pending_signal_replays_once() {
  local home out
  home=$(new_home merged-interrupted-publish)
  forge_home "$home"
  wrap_forge "$home"
  mutate_record "$home" delivery '.records[0] |= (.observation.state = "merged"
    | .seen = ["comment:9"] | .notified = []
    | .pending = [{token:"comment:9",type:"comment",source:"https://github.com/o/r/pull/8#issuecomment-9",head:null,author:"maintainer",body:"Thank you"}])'
  printf 'down\n' > "$home/forge/fault"
  out=$(with_home "$home" "$ROOT/bin/fm-contributions.sh" poll) || fail 'merged pending poll failed'
  assert_contains "$out" 'contribution-wake:' 'the persisted signal was never published'
  with_home "$home" "$ROOT/bin/fm-contributions.sh" poll >/dev/null || fail 'merged pending retry failed'
  [ "$(wc -l < "$home/state/.wake-queue" | tr -d ' ')" = 1 ] || fail 'merged recovery duplicated its signal'
  jq -e '.records[0] | .notified == ["comment:9"] and (.pending | length) == 1' \
    "$home/data/delivery/contributions.json" >/dev/null || fail 'merged signal acknowledgment was lost'
  [ ! -s "$home/forge/calls" ] || fail 'merged replay re-read the forge'
