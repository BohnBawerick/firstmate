...
#   3. Node process titles vary across platforms and versions.
#      Ancestry recognizes node and node-MainThread interpreter titles;
#      MainThread still requires the marker. Native gemini names are covered too.
...
test_gemini_node_bundle_uses_the_actual_interpreter_title() {
  command -v node >/dev/null 2>&1 || return 0
  local dir="$TMP_ROOT/ancestry" out comm
  mkdir -p "$dir"
  # Exercise the real Node process title rather than assuming a platform name.
