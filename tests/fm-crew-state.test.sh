...
test_unfetched_live_sibling_does_not_bind_from_terminal_row() {
  reset_fakes
...
EOF
  ...
  assert_contains "$out" "state: failed" "the verified terminal run remains authoritative without proof of live run ownership"
  pass "an unfetched live sibling cannot bind from a neighboring terminal row"
}
