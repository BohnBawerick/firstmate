...
# or as unreachable.
make_idle_shell_process() {
  local directory=$1
  mkdir -p "$directory/idle-shell-bin"
  ln -s "$(command -v sleep)" "$directory/idle-shell-bin/zsh"
  "$directory/idle-shell-bin/zsh" 300 &
  FM_FAKE_HERDR_SHELL_PID=$!
...
test_unbindable_terminal_run_reports_unknown_not_failed() {
  ...
  assert_not_contains "$out" "source: run-step" "an unverified terminal run cannot supply task state"
...
test_newer_verified_live_run_outranks_terminal_history() {
  make_competing_runs_case newest-live running failed
  local d=$TMP_ROOT/newest-live out
  FM_FAKE_AXI_STATUS="$(run_failed fm/competing | sed 's/01RUN/01OLD/')"
  FM_FAKE_AXI_STATUS_RUN="$(run_running fm/competing | sed 's/01RUN/01NEW/')"
  out=$(run_crew_state "$d" competing)
  assert_contains "$out" 'state: working' 'the newer verified live attempt must win'
  assert_contains "$out" '01NEW' 'current live state names its own run'
  pass 'newer verified live run outranks terminal history'
}

test_unproved_terminal_run_cannot_win_from_adjacent_rows() {
  local mode d out
  for mode in missing-proof foreign-anchor matched-anchor; do
    make_competing_runs_case "terminal-proof-$mode" failed running
    d=$TMP_ROOT/terminal-proof-$mode
    FM_FAKE_AXI_STATUS="$(run_running fm/competing | sed 's/01RUN/01OLD/')"
    FM_FAKE_AXI_STATUS_RUN="$(FM_FAKE_RUN_HEAD=$UNFETCHED_HEAD run_failed fm/competing | sed 's/01RUN/01NEW/')"
    FM_FAKE_AXI_HOME=$(printf '%s\n' "$FM_FAKE_AXI_HOME" | sed "/01NEW/s/,[a-f0-9]*,\"\"$/,$UNFETCHED_HEAD,\"\"/")
    FM_FAKE_RUNS_LIST="failed fm/competing $UNFETCHED_HEAD 2026-09-18 11:20
running fm/competing $FM_FAKE_RUN_HEAD 2026-09-18 10:05"
    case "$mode" in
      foreign-anchor) FM_FAKE_AXI_SYNC=$(sync_pipeline "$FM_FAKE_RUN_HEAD" "$UNFETCHED_HEAD" | sed 's/01RUN/01OLD/') ;;
      matched-anchor) FM_FAKE_AXI_SYNC=$(sync_pipeline "$FM_FAKE_RUN_HEAD" "$UNFETCHED_HEAD" | sed 's/01RUN/01NEW/') ;;
    esac
    out=$(run_crew_state "$d" competing)
    if [ "$mode" = matched-anchor ]; then
      assert_contains "$out" 'state: failed' 'the selected run owns its submitted-head proof'
    else
      assert_contains "$out" 'state: unknown' 'adjacent rows or another run anchor confer no ownership'
      assert_not_contains "$out" 'state: failed' 'unproved terminal record cannot claim failure'
    fi
    pass "$mode terminal identity respects explicit ownership"
  done
}
