...
Herdr's own SSH remote attach can start the server before the Aqua login session exists.
The guard repairs only a proven SSH-born server; unknown origin preserves the running session and its panes.
The doctor likewise refuses a reload when server status or origin cannot be proved.
