...
While a session holds the home, the recorded pid always names a live process, because liveness is the only evidence another process has that the home is held at all and a dead pid reads as a free home fleet-wide.
Inheriting the helm by conversation id is what makes a dead recorded pid reachable, so `bin/fm-lock.sh` rewrites one at every acquisition, and `bin/fm-claude-stop-autoarm.sh` - the only caller that fires on an ordinary turn - reclaims through it whenever it finds one, whether or not this session already owns the home.
