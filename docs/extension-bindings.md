...
Source reconciliation and retirement wait only on that source's own invocations, including its handshake, so another source's active poll on the same binding never blocks them.
