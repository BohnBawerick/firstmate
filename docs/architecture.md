...
Verified activity still suppresses escalation for that lane, while a lane without it is rechecked with `the declared clearing time has passed` before any generic wedge escalation.
A lane that never declared a wait keeps the unchanged escalation schedule, reason and `demand-deep-inspection` wording.
...
The status file's mtime gates the initial reminder; after a reminder is recorded, the declaration-bound throttle alone controls the recheck cadence, so footer updates neither restart the wait nor trigger extra wakes.
