...
Code identity is ternary, so a run head this copy cannot resolve at all - the ordinary shape of a live run whose pipeline-side commits were never fetched here - is unknown identity rather than another crew's code.
Such a head binds only on the branch-custody evidence the run itself carries, strongest first: through `axi status` an active run whose `branch_sync` names `pipeline_owned` binds outright, any other `branch_sync` state is positive evidence against attribution and refuses the run, and a run carrying no `branch_sync` block at all binds as unverified, so it may report that work is still under way but never a terminal passed or failed verdict.
The coarse runs list carries no launch anchor and no custody block, so only a verified match binds there; anything else stops the walk and leaves attribution unknown, and the pane or status log answers instead of a superseded older row.
[`bin/fm-nm-run-lib.sh`](../bin/fm-nm-run-lib.sh)'s header owns the exact branch, head, pipeline-custody, and newest-first attribution primitives, including the pipeline-owned-active exemption, and [`bin/fm-crew-state.sh`](../bin/fm-crew-state.sh) owns what a reporting caller does with each verdict.
