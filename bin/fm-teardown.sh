...
#     When the run head is absent from this copy's object store, attribution
#     requires explicit submitted-head identity or active pipeline custody.
#     Coarse ledger rows never authorize an abort of an unfetched run.
...
remote_secondmate_teardown() {
  ...
  fm_wake_retire_task_markers "$STATE" "$ID" "${T:-}" || return 1
...
# Which adapters mint one is owned there too, so a new adapter of that shape
# cannot be wired without teardown retiring both its registry entry and the
# private state token that entry is named by.
remove_turnend_auth() {
  local state_dir=$1 id=$2 harness token_path token path
...
# binds through object-local identity, an explicit submitted head, or active
# pipeline custody. A coarse ledger cannot prove an unfetched run belongs here.
...
task_status_is_own_parked_run() {  # <worktree> <axi-status-output>
  local wt=$1 out=$2 branch run_id run_branch run_head status outcome awaiting has_gate submitted_head
  TASK_RUN_ID=
  ...
  if ! fm_nm_head_matches_worktree "$wt" "$run_head"; then
    ...
    # adjacent coarse ledger rows never authorize an abort.
    [ -n "$run_head" ] || return 1
...
require_owned_worktree_slot_record() {
  local record_id=$1 worktree=$2 home=$3 marker
  fm_treehouse_slot_owner_state "$worktree" "$record_id" "$home"
  ...
  echo "REFUSED: task $record_id's recorded worktree $worktree has no provable slot-owner claim, so the slot cannot be proved to still be this task's; nothing was changed - not even with --force." >&2
...
require_owned_task_worktree_slot() {
  local slot rc=0
  slot=$(teardown_live_slot_path) || return 0
  require_owned_worktree_slot_record "$ID" "$slot" "$FM_HOME" || rc=$?
  case "$rc" in
    0) return 0 ;;
...
cleanup_firstmate_home_children() {
  ...
  for child_meta in "$sub_state"/*.meta; do
    ...
    remove_turnend_auth "$sub_state" "$child_id" || return 1
...
if [ "$BACKEND" = orca ] && [ "$KIND" != secondmate ]; then
  ...
  if [ -d "$WT" ]; then
    ...
    remove_turnend_worktree_pointers "$WT"
  fi
...
remove_turnend_auth "$STATE" "$ID" || exit 1
...
fm_wake_retire_task_markers "$STATE" "$ID" "$T" || exit 1
rm -f "$STATE/$ID.turn-ended" "$STATE/$ID.progress" \
