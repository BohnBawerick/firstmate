...
effort_flag_for_harness() {
  ...
  case "$harness" in
    ...
    # effort flag. The agy arm above handles suffix-based effort separately.
  esac
}
...
if [ "$KIND" = secondmate ]; then
  ...
  if [ "${FM_SKIP_SECONDMATE_SYNC:-0}" = 1 ]; then
    ...
    sm_ff_out=$(ff_target "$PROJ_ABS" "secondmate $ID" "$sm_primary_head" yes yes "$PROJ_ABS" 2>&1 || true)
    case "$sm_ff_out" in
      *': skipped:'*)
