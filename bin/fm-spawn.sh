...
#   A pooled slot whose only deviation is a stale submodule gitlink is refused by
#   that same clean check, but is reported as a stale checkout naming each
#   submodule and both pins; nothing is converged or removed, and no remedy is
#   suggested. That report is only reached when each submodule's checked-out
#   commit is already contained in one of its remotes, so a submodule carrying an
#   unpushed commit keeps the conservative uncommitted-work refusal instead. That
