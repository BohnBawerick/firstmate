...
harness_process_verdict() {  # <pid>
  ...
  case "$(basename -- "$comm")" in
    ...
    # Node titles vary: node and node-MainThread reach the interpreter arm,
    # while MainThread, measured on Node v24.20.0, requires GEMINI_CLI.
    # Do not broaden interpreter matching to arbitrary thread titles.
    # Gemini remains a crewmate/scout adapter without primary supervision.
