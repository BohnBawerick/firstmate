...
#      Run identity uses the ternary verdict and submitted-head anchor in
#      bin/fm-nm-run-lib.sh. Unverified identity can report in-flight work but
#      never a terminal outcome. The same library owns pipeline custody and
#      creation-ordered run selection and ambiguity reporting.
...
#      neither is the overview table's narrower status column) is authoritative
#      REGARDLESS of head (fm_nm_run_is_executing in bin/fm-nm-run-lib.sh) as
#      long as an explicit probe has not ANSWERED that the daemon is down
#      (nm_daemon_answered_down): the pipeline rebases the branch and commits
#      its fix rounds in its own checkout, so a live run's head routinely
#      differs from the local head, and reading an older run that still matches
#      the local head would report a working crew as failed - but a record
#      still saying `running` because the daemon died under it is evidence from
#      a dead instrument, exactly as for a terminal record, and must not answer
...
#      working. A run PARKED at a gate is exempt from the dead-instrument
#      verdict: an open decision stays open when the instrument dies, so it
#      keeps its gate and findings.
...
nm_submitted_head() {
  if [ "$NM_SUBMITTED_HEAD_PROBED" = 0 ]; then
    ...
    NM_SUBMITTED_HEAD=$(fm_nm_submitted_head "$WT" "$NM_TIMEOUT" "$(strip_quotes "$(nm_field id)")")
  fi
...
# Custody can resolve missing objects, never a proved code mismatch.
nm_run_identity_is_proved() {
  case "$(nm_run_head_identity)" in
    match) return 0 ;;
    unverified) fm_nm_run_is_pipeline_owned_active "$RUN_OUT" ;;
    *) return 1 ;;
  esac
}
      ...
      selected\|*)
        ...
        case "$(nm_run_head_identity)" in
          ...
          unverified)
            ...
            ;;
          unbound) emit unknown run-step "selected run code identity unverified; run ids: $candidate_ids" ;;
          mismatch)
            # Local development moved beyond this recorded run, unless the run
            # is still executing on this branch while the daemon answers: the
            # pipeline's own rebase and fix commits are the ordinary reason a
            # live run's head no longer matches this copy.
            if fm_nm_run_is_executing "$RUN_OUT" && ! nm_daemon_answered_down; then
              HAVE_RUN=1
            fi
            ;;
        esac
