...
#     after STALE_ESCALATE_SECS (configurable) unless its crew is affirmatively
#     provably working. A stale pane whose semantic busy source affirmatively
#     reports working suppresses escalation and resets the timer. A wedged
#     crewmate is therefore detected within STALE_ESCALATE_SECS + a tick, never
#     lost. A declared wait - either a paused: external wait or a verified
#     captain-held transfer, per fm-classify-lib.sh's combined predicate -
#     instead gets its own longer PAUSE_RESURFACE_SECS recheck, never a wedge
#     escalation, whether its pane reads idle or busy; only a status append that
#     stops declaring the wait ends that routing.
...
#     A captain-held transfer is not rechecked while state/.afk-contract
#     exists; the return brief lists it.
...
#     same helper; gone -> clear; still declaring the wait, on an idle, busy or
#     unreadable pane -> escalate a recheck digest naming which human the wait is
...
housekeeping() {  # <state>
  ...
  for marker in "$state"/.subsuper-stale-*; do
    ...
    if [ -n "$task" ] && crew_is_provably_working "$task"; then
      ...
      case "$verdict" in
        ...
        unreadable)
          if escalate_add "$state" "stale persisted ${age}s (pane present but unreadable): $win"; then
            _now > "$marker"
          fi
          ;;
        *)
  ...
  # An unreadable pane is kept and named rather than dropped: the watcher cannot
  # recapture it, so retiring the marker would forget the wait entirely.
  ...
  for marker in "$state"/.subsuper-paused-*; do
    ...
    # busy for as long as the wait lasts, and migrate_watcher_pause_markers would
    # recreate the marker with a fresh timestamp on the very next tick while the
    # declaration still stands, so the window would restart forever and the wait
    # would never mature into its one recheck. Do NOT reinstate a `busy)` arm.
    # An unreadable pane is kept and named, because the watcher cannot recapture
    # it and dropping the marker would forget it.
    verdict=$(stale_window_recheck "$win" "$state")
    case "$verdict" in
      gone) rm -f "$marker" ;;
      *)
        ...
        if [ -n "$last" ] && status_is_captain_held "$last"; then
          ...
          if [ "$verdict" = unreadable ]; then
            pause_reason="paused ${age}s (pane present but unreadable, recheck whether the wait still holds): $win"
          elif [ "$bounded_until" -eq 1 ]; then
...
handle_wake() {  # <reason> <state>
  ...
  case "$reason" in
    ...
    stale:*)  kind=stale; arg="${reason#stale: }"; stale_detail="${arg#"$arg"}"
              ...
              # An affirmatively provably-working crew is the other exception: the
              # demand not to re-absorb on pane state does not reach a positive
              # busy-source reading, which is evidence the pane is not wedged.
              case "${decision%%|*}" in
                pause) : ;;
                *) case "$stale_detail" in
                     idle\ *s,\ possible\ wedge,\ escalation\ *)
                       ...
                       if status_is_paused_or_captain_held "$last"; then
                         :
                       elif [ -n "$task" ] && crew_is_provably_working "$task"; then
