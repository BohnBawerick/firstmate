...
crew_dispatch_validate() {
  ...
  err=$(jq -r '
    def verified($h): ["claude","codex","opencode","pi","pi-signed","grok","kimi","cursor","gemini","agy","muse","rovo","omp"] | index($h);
    def effort_ok($h; $m; $e):
      if $e == null then true
      ...
      elif $h == "opencode" or $h == "kimi" or $h == "cursor" or $h == "gemini" then false
      else true
      end;
    def profiles($value):
