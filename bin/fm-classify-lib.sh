...
_fm_status_event_scan() {
  local line last='' prev='' fallback='' verb legacy_re matched unstamped
  ...
  while IFS= read -r line || { newline=0; [ -n "$line" ]; }; do
    ...
    case "$verb" in
      ...
      *) _fm_status_unstamped "$line" unstamped
         _fm_classify_matches "$unstamped" "$legacy_re" && matched=1 ;;
    esac
    if [ "$matched" = 1 ]; then
      prev=$last
      last=$line
...
# every non-working verdict still follows the stale escalation schedule.
