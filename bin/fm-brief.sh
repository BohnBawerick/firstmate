...
#   --dreamer writes the ephemeral dreamer scout contract: the deliverable is a
#   proposed immutable memory generation under data/memory/gen/<N>/ plus a dream
#   receipt report. The dreamer reads the append-only log and the drop tray,
#   never writes published memory in place, never takes the session lock, and
#   never addresses the captain. It may be combined with --herdr-lab.
...
The main firstmate's answer normally writes that closing line at answer time; when a blocker or wait clears WITHOUT an answer from the main firstmate, append \`resolved [key=<slug>] [at=<epoch>]: {how it cleared}\` yourself (same \`[key=<slug>]\` if you opened it with one) as your domain resumes.
...
6. If a decision belongs to a human (product choices, destructive actions),
   ...
   Firstmate's reply normally writes that closing line at answer time; when a blocker or wait clears WITHOUT a firstmate reply, append \`resolved [key=<slug>] [at=<epoch>]: {how it cleared}\` yourself (same \`[key=<slug>]\` if you opened it with one) as you resume.
...
if [ "$KIND" = dreamer ]; then
cat > "$BRIEF" <<EOF
You are a crewmate: an autonomous worker agent managed by firstmate. Work on your own; do not wait for a human.

# Task
You are the DREAMER: an offline memory-consolidation pass for the firstmate home at $FM_HOME.
...
Before reporting done, read and follow \`$FM_ROOT/.agents/skills/decision-hold-lifecycle/SKILL.md\` and pass its
...
EOF
echo "scaffolded: $BRIEF (dreamer; replace {TASK})"
exit 0
fi
...
DOD=${DOD/"Delivery contract: mode=$MODE"/"$CONTRACT_LINES"}
...
echo "scaffolded: $BRIEF (ship, mode=$MODE$QUALITY_NOTE; replace {TASK} and {FIRSTMATE_SPEC})"
