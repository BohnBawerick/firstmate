...
family_for_basename() {
  case "$1" in
    ...
    fm-watcher-lock.test.sh|fm-inactive-reconcile.test.sh)
      printf '%s\n' watcher-wake-lock
      ;;
    fm-afk-inject-herdr-e2e.test.sh|fm-afk-inject-self-deadlock-e2e.test.sh|fm-afk-launch.test.sh|fm-backend-autodetect-smoke.test.sh|\
    ...
    fm-dreamer.test.sh|fm-memory-compile.test.sh|fm-memory-verify.test.sh|\
    ...
    fm-teardown-endpoint-safety.test.sh)
      printf '%s\n' backend-dispatch
      ;;
    fm-check-unregister.test.sh|fm-pr-check-security.test.sh|fm-pr-merge.test.sh|fm-merge-local.test.sh|\
...
tests/fm-ci-workflow.test.sh 3153
tests/fm-pr-state.test.sh 798
tests/fm-pr-reviewers.test.sh 285
EOF
}
...
families_for_changed_path() {
  ...
  case "$path" in
    ...
    bin/fm-test-run.sh)
      ...
      printf '%s\n' pure-contract-unit
      # Only this script wraps each suite in run_script_contained's fixture Git
    ...
    bin/fm-test-isolation-proof.sh)
      ...
      # through run_script_contained, so it cannot regress fixture Git isolation.
      printf '%s\n' pure-contract-unit
      ;;
    ...
    tests/assets/*)
      ...
      ;;
    tests/*)
      families_for_test_reference "$(basename "$path")" || printf '%s\n' "__unmapped__:$path"
      ;;
...
# Plain --changed and explicit script lists use bounded concurrency; numeric
# --jobs retains the strict all-script admission rule below.
AUTO_CONCURRENCY=0
if { [ "$MODE" = changed ] || [ "$MODE" = scripts ]; } && [ "$JOBS_EXPLICIT" -eq 0 ] && [ "${#SCRIPTS[@]}" -gt 0 ]; then
  auto_admissible=0
  for s in "${SCRIPTS[@]}"; do
