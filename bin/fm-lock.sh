  ...
  case "$FM_LOCK_INSPECT_STATE" in
    ...
    held)
      if fm_session_lock_owned_by_self "$STATE"; then
        echo "lock: held by THIS session (harness pid $FM_LOCK_INSPECT_PID)"
      else
        echo "lock: held by ANOTHER live session (harness pid $FM_LOCK_INSPECT_PID)"
      fi
      ;;
...
# signal that granted it when the recorded pid is not this session's own anchor.
report_acquired() {  # <recorded-pid>
  local pids pid
  if [ "$1" = "$me" ]; then
...
EOF
  ...
  echo "lock acquired: THIS session holds the fleet lock (recorded harness pid $1, same Claude session as this session)"
}
...
refuse_live_owner() {  # <recorded-pid>
  ...
  if recorded=$(fm_session_lock_recorded_session_id "$STATE"); then
    echo "error: NOT THIS SESSION - another live firstmate session holds the lock (pid $1, session $recorded); operate read-only until resolved" >&2
  else
    echo "error: NOT THIS SESSION - another live firstmate session holds the lock (pid $1); operate read-only until resolved" >&2
  fi
  exit 1
}
