...
#   shell      - one recognized idle foreground shell, with no live harness
#                below the exact pane root and a complete, non-zombie lineage.
#                The strict departure sample validates the final shell proof.
...
#   unreadable - the pane identity, foreground, process state, ancestry, or
#                descendant arguments cannot be verified. An empty foreground
#                list remains inconclusive, including during an exec handoff.
...
fm_backend_herdr_pane_process_state_sample() {  # <session> <pane_id>
  ...
  local others=0 ps_bin rows state tree_state foreground_pid
  ...
  rows=$(LC_ALL=C "$ps_bin" -axo pid=,ppid=,stat=,comm= 2>/dev/null) || { printf 'unreadable'; return 0; }
  foreground_pid=$(printf '%s' "$info" | jq -r '.result.process_info.foreground_processes[0].pid // empty')
  tree_state=$(printf '%s\n' "$rows" | fm_backend_herdr_process_tree_pi_state "$shell_pid" "$foreground_pid")
  case "$tree_state" in
    live) printf 'agent'; return 0 ;;
    absent) ;;
    *) printf 'unreadable'; return 0 ;;
  esac
  while IFS=$'\t' read -r pid state name; do
    [ -n "$pid" ] || continue
    case "$state" in Z*|X*) continue ;; esac
    if [ "$(fm_agent_process_classify "$name" '' '' "$pid")" = agent ]; then
      printf 'agent'
      return 0
    fi
...
$(printf '%s\n' "$rows" | awk -v shell="$shell_pid" '
  ...
  END {
    ...
    for (n = 1; n <= NR; n++) {
      if ((pid[n] in want) && pid[n] != shell) printf "%s\t%s\t%s\n", pid[n], state[n], comm[n]
    }
  }')
EOF
  # A missing argument read or incomplete shell lineage never proves exit.
  case "$(fm_backend_herdr_departed_pi_sample "$session" "$pane_id" "$info" "$rows")" in
    departed) printf 'shell' ;;
    live) printf 'agent' ;;
    *) printf 'unreadable' ;;
  esac
}
