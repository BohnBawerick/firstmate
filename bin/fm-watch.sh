...
#                          threshold is rechecked: if its crew is still provably
#                          working the escalation is suppressed and the timer
#                          reset, otherwise it surfaces with an "escalation N"
...
# An established scoped declaration uses the throttle age between reminders.
# Before its first reminder, or for an unscoped deferral, the absorb age also gates
# the wake. An optional <min-age> replaces that initial gate (0 lets a declared
# `until` time that has just passed re-surface at once).
# wake() exits the cycle, exactly as it does inline.
resurface_absorbed() {  # <window> <throttle-marker> <age> <reason> [scope] [min-age]
  ...
  if [ -n "$scope" ] && [ -e "$throttle" ]; then
    # An established declaration uses only its last reminder as the cadence clock.
    # A replacement declaration gets its own first surface.
...
# semantic busy recheck (crew_is_provably_working), and the dead-record probe
...
# on an ordinary poll. The crew-state read wedge_wait_evidence may take under
...
# deferral below; an unconfigured home never reaches that read at all. The wait
# consult runs first, because a pane that can account for its own quiet has
# nothing to prove through its worktree. The semantic busy recheck follows the
# write probe: if the busy source affirmatively reports working, the
# possible-wedge escalation is suppressed and the timer reset. The dead-record
# probe runs last, so the cheaper deferrals keep the panes they already own on
...
# cheap: it NEVER re-reads crew state. The status file mtime gates the first
# reminder; the declaration-bound .paused-resurfaced-<key> marker alone owns
# the cadence after that. Pane churn cannot reset either clock.
# The shared resurface_absorbed helper publishes each bounded reminder.
...
# repeatedly trigger the initial wake. handle_paused_stale owns declaration-bound reminder timing.
