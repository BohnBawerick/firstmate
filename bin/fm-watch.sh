  ...
  case "$since" in
    ...
    *)
      ...
      if [ "$age" -ge "$STALE_ESCALATE_SECS" ]; then
        # Three gates before an escalation, cheapest evidence first: a worktree
        # write since the window opened defers, then a crew that is provably
        # working ages the timer instead of escalating, and only a window that
        # passes both escalates.
